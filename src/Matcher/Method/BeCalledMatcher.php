...
class BeCalledMatcher implements ExpectationMatcherInterface
{
    public function supports(string $methodName): bool
    {
        return in_array($methodName, ['shouldBeCalled', 'shouldNotBeCalled'], true);
    }
    ...
    public function checkExpectation(Expectation $expectation): void
    {
        ...
        if ($negated && $count > 0) {
            throw new RuntimeException("Expected method \"{$expectation->getSubject()->getMethodName()}\" not to be called, but was called $actual.");
        }

        if (!$negated && $expected !== null && $count !== $expected) {
            throw new RuntimeException("Expected method \"{$expectation->getSubject()->getMethodName()}\" to be called $expected times, but was called $actual.");
        }

        if (!$negated && $expected === null && $count === 0) {
            throw new RuntimeException("Expected method \"{$expectation->getSubject()->getMethodName()}\" to be called at least once, but it was not.");
        }
    }
}